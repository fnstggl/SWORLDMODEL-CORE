...
def test_renaming_an_actor_changes_its_grounded_identity() -> None:
    corpus = copy.deepcopy(base_corpus())
    corpus["world_spec"]["entities"][1]["name"] = "Original Person"
    before = _profiles(corpus)["b"]
    assert "Original Person" in before.render_grounding()

    corpus["world_spec"]["entities"][1]["name"] = "Renamed Person"
    after = _profiles(corpus)["b"]
    assert after.canonical_identity == "Renamed Person"
    assert "Renamed Person" in after.render_grounding()
    # the previous identity is not silently retained
    assert "Original Person" not in after.render_grounding()
...
def test_grounding_works_for_a_non_committee_process() -> None:
    ...
    # An individual-response world: the outcome is one actor acting, and the terminal is
    # a declarative predicate over events — there is no mechanism family to name.
    assert compiled.spec.terminal.yes_when.op == "greater_or_equal"
    assert "reply" in compiled.spec.terminal.description.lower()
