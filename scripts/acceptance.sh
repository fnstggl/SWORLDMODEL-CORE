...
set -u
# Resolve the checkout from this script instead of depending on the path used by a
# previous container/session. This is intentionally computed before any artifacts are
# written so a verification run cannot silently execute another checkout.
