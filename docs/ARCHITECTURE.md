...
forecast(question, as_of, horizon, config)          # api.py — the single entry point
  → research            live_research.py  → research.py (types) + evidence.py (store)
...
| The one universal event runtime            | `engine.py`           |
...
See `docs/` for the reality-integrity gate, the evidence/cutoff model, the actor runtime,
forecast semantics, the live product, and the Banxico evaluation.
